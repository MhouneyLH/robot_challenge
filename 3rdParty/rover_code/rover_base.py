...
class Rover:
    ...
    def set_motor_speed(self, speed_A, speed_B):
        ...
        self._client.publish(f'robot/{self.id}/motor', json.dumps(payload), qos=1)
    ...
    def set_servo1_pos(self, pos):
        ...
        self._client.publish(f'robot/{self.id}/servo', json.dumps(self.servos), qos=1)
        time.sleep(1)
    ...
    def set_leds(self, top_1, top_2, side):
        ...
        self._client.publish(f'robot/{self.id}/leds', json.dumps(payload), qos=1)
        time.sleep(1)
        ...
        if(dist<threshold):
            ...
            if (self.position['h']>=0 and goal_heading>0):
                ...
                if abs(heading_error)>180:
                    heading_error = np.sign(goal_heading)*(360-abs(heading_error))
                    #print("fall 4")
    ...
    def _head_to(self, threshold, turn_rate):
        ...
        if (self.position['h']>=0 and self.goal_heading>0):
            ...
            #print("fall 3")
            if abs(heading_error)>180:
                heading_error = np.sign(self.goal_heading)*(360-abs(heading_error))
                #print("fall 4")
        ...
        if(abs(heading_error)<threshold):
            ...
            turn_speed = np.sign(heading_error)*np.sqrt(abs(heading_error)) * KP_turn
            ...
            if abs(turn_speed)<25:
                turn_speed = np.sign(turn_speed)*25 + np.sum(self._heading_error)*KI_turn
